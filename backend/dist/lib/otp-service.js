    ...
    async issue(userId, purpose) {
        ...
        if (latest && !latest.used && !isOtpExpired(latest.expiresAt)) {
            ...
            if (retryAfterSeconds > 0) {
                throw new AppError('OTP_RESEND_COOLDOWN', 'Please wait before requesting another code.', 429, { retryAfterSeconds });
            }
            if (latest.resendCount >= config.otpMaxResendAttempts) {
                throw new AppError('OTP_RESEND_LIMIT', 'You have reached the resend limit. Please wait for the current code to expire.', 429, { retryAfterSeconds: Math.max(1, Math.ceil((latest.expiresAt.getTime() - now.getTime()) / 1_000)) });
            }
