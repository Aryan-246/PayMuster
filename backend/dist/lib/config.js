...
const documentAllowedMimeTypes = (process.env.DOCUMENT_ALLOWED_MIME_TYPES ??
    ...
    .filter(Boolean);
if (documentAllowedMimeTypes.length === 0 ||
    documentAllowedMimeTypes.some((value) => !knownDocumentMimeTypes.has(value))) {
    throw new Error('DOCUMENT_ALLOWED_MIME_TYPES must contain only supported document formats (PDF, JPEG, PNG, or Office documents).');
}
...
export const config = Object.freeze({
    ...
    geminiTimeoutMs: parsePositiveInteger('GEMINI_TIMEOUT_MS', process.env.GEMINI_TIMEOUT_MS, 15_000),
    ...
    documentUploadMaxBytes: parsePositiveInteger('DOCUMENT_UPLOAD_MAX_BYTES', process.env.DOCUMENT_UPLOAD_MAX_BYTES, 10 * 1024 * 1024),
    documentSignedUrlTtlSeconds: parsePositiveInteger('DOCUMENT_SIGNED_URL_TTL_SECONDS', process.env.DOCUMENT_SIGNED_URL_TTL_SECONDS, 300),
    avatarUploadMaxBytes: parsePositiveInteger('AVATAR_UPLOAD_MAX_BYTES', process.env.AVATAR_UPLOAD_MAX_BYTES, 5 * 1024 * 1024),
...
function readiness(enabled, configured, supported) {
    if (!enabled)
        return 'DISABLED';
    if (!supported)
...
function providerSummary(provider, kind, enabled, configured, supported, fallback, testMode) {
    return {
        provider,
        kind,
        enabled,
